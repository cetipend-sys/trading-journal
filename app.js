...
function App() {
  const [page, setPage] = useLocalStorage('page', 'dashboard');
  const [trades, setTrades] = useLocalStorage('trades', SAMPLE_TRADES);
  const [mistakes, setMistakes] = useLocalStorage('mistakes', []);
  const [eowReviews, setEowReviews] = useLocalStorage('eowReviews', []);
  const [showAdd, setShowAdd] = useState(false);
  const [selectedTrade, setSelectedTrade] = useState(null);
  const [calMonth, setCalMonth] = useState(new Date(2026, 3));
